...
quality due to editing.@note{Digital editors achieve this
 ...
 operating on the videos themselves.} An NLVE
is a graphical tool with a @emph{time line} comprised
...
tracks simultaneously and applying filters.
...
Stringent macros are extremely brittle and frequently break, even
within a single application, because they do not understand
the underlying tools. Using a tool's official plug-in
interface produces more robust scripts, but leads to a
...
when combining high interactive and programmatic work flows.
